<script setup>
import { TrendingUp, ExternalLink, Star } from 'lucide-vue-next'
...
<template>
  <article class="lg:col-span-2 bg-white rounded-2xl border border-gray-100 p-5 md:p-6 shadow-[0_1px_3px_0_rgba(0,0,0,0.02)] hover:shadow-sm transition-shadow duration-300">
          ...
          <tr v-for="(product, index) in products" :key="product.id"
            ...
            <td class="py-3.5 pl-5 md:pl-6">
              <span :class="[
                ...
                index === 0 ? 'bg-linear-to-br from-yellow-300 to-yellow-500 text-white shadow-sm' :
                  index === 1 ? 'bg-linear-to-br from-gray-300 to-gray-400 text-white shadow-sm' :
                    index === 2 ? 'bg-linear-to-br from-amber-600 to-amber-700 text-white shadow-sm' :
            ...
            <td class="py-3.5">
              <div class="flex items-center gap-2">
                <span class="text-sm font-medium text-gray-950 group-hover:text-primary transition-colors">{{ product.name }}</span>
                <ExternalLink class="w-3.5 h-3.5 text-gray-300 group-hover:text-primary transition-colors" />
              </div>
            </td>
            <td class="py-3.5">
              <span class="inline-flex px-2 py-0.5 rounded-md bg-gray-50 border border-gray-100 text-xs font-medium text-gray-600">
                {{ product.category }}
              </span>
            </td>
