...
async function run() {
  ...
  console.log("Iniciando Puppeteer para listar preços principais (excluindo carrosséis)...");
  ...
  console.log("\n=== PREÇOS QUE NÃO ESTÃO EM CARROSSÉIS OU RECOMENDAÇÕES ===");
  $(".andes-money-amount").each((i, el) => {
    const $el = $(el);
    
    // Verifica se está dentro de carrossel, recomendações ou similares
    const isRecommendation = $el.closest(".ui-recommendations-carousel-free, .recos-polycard, .poly-card, .andes-carousel-free").length > 0;
